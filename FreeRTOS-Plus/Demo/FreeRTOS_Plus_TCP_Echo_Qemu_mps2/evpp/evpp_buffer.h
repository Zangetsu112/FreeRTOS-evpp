...
public:
    ...
    explicit Buffer(size_t initial_size = kInitialSize, size_t reserved_prepend_size = kCheapPrependSize)
        ...
        configASSERT(WritableBytes() == initial_size);
        configASSERT(PrependableBytes() == reserved_prepend_size);
    }
    ...
    void EnsureWritableBytes(size_t len) {
        ...
        configASSERT(WritableBytes() >= len);
    }
...
public:
    void Write(const void* /*restrict*/ d, size_t len) {
        ...
        configASSERT(write_index_ + len <= capacity_);
        write_index_ += len;
    }
    ...
    void AppendInt32(int32_t x) {
        int32_t be32 = FreeRTOS_htonl(x);
        Write(&be32, sizeof be32);
    }

    void AppendInt16(int16_t x) {
        int16_t be16 = FreeRTOS_htons(x);
    ...
    void Prepend(const void* /*restrict*/ d, size_t len) {
        configASSERT(len <= PrependableBytes());
        read_index_ -= len;
    ...
    void UnwriteBytes(size_t n) {
        configASSERT(n <= length());
        write_index_ -= n;
    }

    void WriteBytes(size_t n) {
        configASSERT(n <= WritableBytes());
        write_index_ += n;
    }
...
public:
    ...
    int64_t PeekInt64() const {
        configASSERT(length() >= sizeof(int64_t));
        int64_t be64 = 0;
    ...
    int32_t PeekInt32() const {
        configASSERT(length() >= sizeof(int32_t));
        int32_t be32 = 0;
        ::memcpy(&be32, data(), sizeof be32);
        return FreeRTOS_ntohl(be32);
    }

    int16_t PeekInt16() const {
        configASSERT(length() >= sizeof(int16_t));
        int16_t be16 = 0;
        ::memcpy(&be16, data(), sizeof be16);
        return FreeRTOS_ntohs(be16);
    }

    int8_t PeekInt8() const {
        configASSERT(length() >= sizeof(int8_t));
        int8_t x = *data();
        return x;
    }
...
public:
    ...
    size_t length() const {
        configASSERT(write_index_ >= read_index_);
    ...
    size_t WritableBytes() const {
        configASSERT(capacity_ >= write_index_);
...
public:
    ...
    const char* FindCRLF(const char* start) const {
        configASSERT(data() <= start);
        configASSERT(start <= WriteBegin());
